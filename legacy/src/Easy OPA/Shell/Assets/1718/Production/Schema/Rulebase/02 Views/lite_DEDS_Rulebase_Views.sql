...
as
	select	g.UKPRN,
			...
			spv.Period_11,
			spv.Period_12
	from	Rulebase.SFA_LearningDelivery_PeriodisedValues as spv
...
as
	...
	from	Rulebase.SFA_LearningDelivery_Period as sp
				cross join (select top 1 UKPRN from Rulebase.SFA_global) as g
go
...
as 
	...
	from	Rulebase.SFA_LearningDelivery as sld
...
as
	select	g.UKPRN,
			...
			el.StartFund,
			el.ThresholdDays
	from	Rulebase.EFA_Learner as el
...
as
	select	g.UKPRN,
			...
			espv.Period_11,
			espv.Period_12
	from	Rulebase.EFA_SFA_Learner_PeriodisedValues as espv
...
as
	select	g.UKPRN,
			...
			esp.[Period],
			esp.LnrOnProgPay
	from	Rulebase.EFA_SFA_Learner_Period as esp
				cross join (select top 1 UKPRN from Rulebase.EFA_SFA_global) as g
go
