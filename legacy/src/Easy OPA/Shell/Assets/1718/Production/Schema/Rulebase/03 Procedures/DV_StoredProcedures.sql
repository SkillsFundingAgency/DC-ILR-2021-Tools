	...
	begin
		...
		select
			...
			convert(xml,
				(
					select
						[Org_Details].[ThirdSector] as [@ThirdSector],
						[LearningProvider].[UKPRN] as [@UKPRN],
						(
							select
								[Learner].[DateOfBirth] as [@DateOfBirth],
								[EFA_Learner].[FundLine] as [@EFA_CalcFundingLineType],
								[EFA_Learner].[StartFund] as [@EFA_CalcStartFunding],
								[EFA_Learner].[ThresholdDays] as [@EFA_CalcThresholdDays],
								...
								(
									select
										...
										les.[ESMCode_BSI] as [@EmpStatMon_BSI],
										les.[ESMCode_EII] as [@EmpStatMon_EII],
										les.[ESMCode_LOE] as [@EmpStatMon_LOE],
										les.[ESMCode_LOU] as [@EmpStatMon_LOU],
										les.[ESMCode_PEI] as [@EmpStatMon_PEI],
										les.[ESMCode_SEI] as [@EmpStatMon_SEI],
										les.[ESMCode_SEM] as [@EmpStatMon_SEM]
									from
											Valid.LearnerEmploymentStatusDenormTbl les
									where
											les.[LearnRefNumber] = [Learner].[LearnRefNumber]
								...
								(
									...
									where
										[ContactPreference].[LearnRefNumber] = [Learner].[LearnRefNumber]
								...
								(
									select
										...
										[SFA_LearningDelivery].[LargeEmployerSFAFctr] as [@SFA_CalcLargeEmpFactor],
										[SFA_LearningDelivery].[FundStart] as [@SFA_CalcStartFunding],
										[SFA_LearningDelivery].[ThresholdDays] as [@SFA_CalcThresholdDays],
										...
										(
											select
												[LARS_AnnualValue].[BasicSkills] as [@BasicSkills],
												[LARS_AnnualValue].[BasicSkillsParticipation] as [@BasicSkillsParticipation],
												[LARS_AnnualValue].[BasicSkillsType] as [@BasicSkillsType],
												[LARS_AnnualValue].[EffectiveFrom] as [@EffectiveFrom],
												[LARS_AnnualValue].[EffectiveTo] as [@EffectiveTo],
												...
												[LARS_AnnualValue].[SfaApprovalStatus] as [@SFAApprovalStatus]
											from
										...
										(
											...
											where
												[LearningDeliveryHE].[LearnRefNumber] = ld.[LearnRefNumber]
												and [LearningDeliveryHE].[AimSeqNumber] = ld.[AimSeqNumber]
										...
										(
											...
											where
												[LARS_Validity].[LearnAimRef]=ld.[LearnAimRef]
											for xml path ('LearningDeliveryLARSValidity'), type
										)
									from		Valid.LearningDeliveryDenormTbl ld

									left join	[Rulebase].[ALB_LearningDelivery]
										   on	[ALB_LearningDelivery].[LearnRefNumber]=ld.[LearnRefNumber]
									...
									left join	[Reference].[LARS_Section96]
										   on	[LARS_Section96].[LearnAimRef]=ld.[LearnAimRef]
									left join	[Rulebase].[SFA_LearningDelivery]
										   on	[SFA_LearningDelivery].[LearnRefNumber]=ld.[LearnRefNumber]
										  and	[SFA_LearningDelivery].[AimSeqNumber]=ld.[AimSeqNumber]
									where
								...
								(
									...
									where
										[LLDDandHealthProblem].[LearnRefNumber] = [Learner].[LearnRefNumber]
		...
		from
			[Valid].[Learner] ControllingTable	end
GO
