IF OBJECT_ID('GetBursaryFundingForType') IS NOT NULL
BEGIN
    DROP FUNCTION  GetBursaryFundingForType
END
GO
CREATE FUNCTION GetBursaryFundingForType (@learnerType VARCHAR(MAX), @collectionPeriod int)
RETURNS TABLE
AS 
RETURN
(
 ...
 (FundCalcApr + FundCalcMay + FundCalcJun + FundCalcJul) as AprToJul, * FROM (
	...
	SELECT 
		...
		WHERE sfaf.FundLine = @learnerType and sfap.AttributeName='ALBSupportPayment'
