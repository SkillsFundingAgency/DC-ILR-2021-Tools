...
begin
	...
	declare @UKPRN int         
	select	@UKPRN = UKPRN
	from	(select top 1 UKPRN from Valid.LearningProvider) as T
    ...
    select	cntrl.[LearnRefNumber],
			convert(xml, (	select	lv.[CurrentVersion] as [@LARSVersion],
									...
									(select	l.[DateOfBirth] as [@DateOfBirth],
											...
											(select	ld.[AchDate] as [@AchDate],
                                                    ...
                                                    (	select	[LearningDeliveryFAM].[LearnDelFAMCode] as [@LearnDelFAMCode],
														...
														where	[LearningDeliveryFAM].[LearnRefNumber] = ld.[LearnRefNumber]
														and		[LearningDeliveryFAM].[AimSeqNumber] = ld.[AimSeqNumber]
														for xml path ('LearningDeliveryFAM'), type),
                                                    (	select	[LARS_LearningDeliveryCategory].[EffectiveFrom] as [@LearnDelCatDateFrom],
														...
														where	[LARS_LearningDeliveryCategory].[LearnAimRef]=ld.[LearnAimRef]
														for xml path ('LearningDeliveryLARSCategory'), type),
                                                    (	select	[LARS_AnnualValue].[BasicSkillsType] as [@LearnDelAnnValBasicSkillsTypeCode],
														...
														where	[LARS_AnnualValue].[LearnAimRef]=ld.[LearnAimRef]
														for xml path ('LearningDeliveryAnnualValue'), type),
                                                    (	select	[LARS_Funding].[FundingCategory] as [@LARSFundCategory],
														...
														where	[LARS_Funding].[LearnAimRef]=ld.[LearnAimRef]
											...
											(	select	[LearnerEmploymentStatus].[DateEmpStatApp] as [@DateEmpStatApp],
														...
														(	select	[LargeEmployers].[EffectiveFrom] as [@LargeEmpEffectiveFrom],
																	[LargeEmployers].[EffectiveTo] as [@LargeEmpEffectiveTo]
															from	[Reference].[LargeEmployers]
															where	[LargeEmployers].[ERN] = [LearnerEmploymentStatus].[EmpId]
															for xml path ('LargeEmployerReferenceData'), type)
												from	[Valid].[LearnerEmploymentStatus]
												where	[LearnerEmploymentStatus].[LearnRefNumber] = l.[LearnRefNumber]
									...
									(	select	[Org_Funding].[EffectiveFrom] as [@OrgFundEffectiveFrom],
