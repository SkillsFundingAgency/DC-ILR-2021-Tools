IF OBJECT_ID('GetTrailblazerLSFFundingForType') IS NOT NULL
BEGIN
    DROP FUNCTION  GetTrailblazerLSFFundingForType
END
GO
CREATE FUNCTION GetTrailblazerLSFFundingForType (@learnerType VARCHAR(MAX), @collectionPeriod int) 
RETURNS TABLE 
AS 
RETURN
(
 ...
 (FundCalcApr + FundCalcMay + FundCalcJun + FundCalcJul) as AprToJul,* FROM (
	...
	SELECT
		...
		WHERE tbf.FundLine = @learnerType and tbfp.AttributeName in ('LearnSuppFundCash')
