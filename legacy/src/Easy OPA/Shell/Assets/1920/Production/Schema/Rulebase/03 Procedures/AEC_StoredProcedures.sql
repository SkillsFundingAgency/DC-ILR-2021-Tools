	...
	select	ControllingTable.[LearnRefNumber],
			convert(xml, (select	lp.UKPRN as [@UKPRN],
									...
									(select l.LearnRefNumber as [@LearnRefNumber],
											...
											(select	LearnRefNumber as [@HistoricLearnRefNumberInput],
											...
											(select	ld.AimSeqNumber as [@AimSeqNumber],
													...
													(select	FundingCategory as [@StandardAFFundingCategory],
													...
													(select	FundingCategory as [@FrameworkAFFundingCategory],
...
begin
	...
	select	LearnRefNumber,
			...
			max(case AttributeName when 'LearnDelSEMContWaiver' then Value else null end) as LearnDelSEMContWaiver,	
	...
	set		FundLineType = SecondPass.FundLineType,
			LearnDelContType = SecondPass.LearnDelContType
	from	Rulebase.AEC_LearningDelivery_Period as FirstPass
				inner join (select	LearnRefNumber,
					...
					on FirstPass.LearnRefNumber = SecondPass.LearnRefNumber
