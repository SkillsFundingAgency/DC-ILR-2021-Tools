...
begin
	insert into Rulebase.TBL_Cases (
		LearnRefNumber,
		CaseData
	)
	select	Filter_LearningDelivery.LearnRefNumber,
			convert(xml, (	select	LARS_Current_Version.CurrentVersion as [@LARSVersion],
									...
									(select	Learner.DateOfBirth as [@DateOfBirth],
											...
											(select	LearningDelivery.AchDate as [@AchDate],
													...
													(select	AppFinRecord.AFinAmount as [@AFinAmount],
													...
													(select	LARS_StandardCommonComponent.CommonComponent as [@LARSCommonComponent],
													...
													where	LARS_StandardCommonComponent.StandardCode = LearningDelivery.StdCode
													for xml path ('LARS_StandardCommonComponent'), type),
													(select	LARS_StandardFunding.FundableWithoutEmployer as [@FundableWithoutEmployer],
													...
													where	LARS_StandardFunding.StandardCode = LearningDelivery.StdCode
									...
									from	Valid.Learner
									where	Learner.LearnRefNumber = Filter_LearningDelivery.LearnRefNumber
	...
	from	(select	distinct
			...
			and		LearningDelivery.FundModel = 81) as Filter_LearningDelivery
end
go
...
begin
	insert into Rulebase.TBL_global (
		UKPRN,
		CurFundYr,
		LARSVersion,
		RulebaseVersion
...
begin
	set nocount on
	insert into	[Rulebase].[TBL_LearningDelivery] (
		LearnRefNumber,
		AimSeqNumber,
...
begin
	insert into [Rulebase].[TBL_LearningDelivery_PeriodisedValues] (
		LearnRefNumber,
		AimSeqNumber,
		AttributeName,
		Period_1,
...
begin
	...
	insert into [Rulebase].[TBL_LearningDelivery_Period] (
		LearnRefNumber,
		AimSeqNumber,
		[Period],
		AchPayment,
	...
	select	LearnRefNumber,
			AimSeqNumber,
			[Period],
			max(case AttributeName when 'AchPayment' then Value else null end) as AchPayment,
			max(case AttributeName when 'CoreGovContPayment' then Value else null end) as CoreGovContPayment,
			max(case AttributeName when 'CoreGovContUncapped' then Value else null end) as CoreGovContUncapped,
			...
			max(case AttributeName when 'SmallBusPayment' then Value else null end) as SmallBusPayment,
			max(case AttributeName when 'YoungAppFirstPayment' then Value else null end) as YoungAppFirstPayment,
			max(case AttributeName when 'YoungAppPayment' then Value else null end) as YoungAppPayment,
			max(case AttributeName when 'YoungAppSecondPayment' then Value else null end) as YoungAppSecondPayment
	from	(	select	LearnRefNumber,
				...
				from	Rulebase.TBL_LearningDelivery_PeriodisedValues
							unpivot ([Value] for [Period] in (Period_1, Period_2, Period_3, Period_4, Period_5, Period_6, Period_7, Period_8, Period_9, Period_10, Period_11, Period_12)) as PeriodValue) as TBLPeriodValues
	group by
