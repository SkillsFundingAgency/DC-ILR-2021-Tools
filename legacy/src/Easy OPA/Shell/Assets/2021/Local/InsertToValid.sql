...
BEGIN
	...
	ELSE
		SELECT @output = COALESCE(@output,'') + N'INSERT INTO [Valid].[LearningProvider] ([UKPRN]) SELECT DISTINCT UKPRN FROM ['+@schema+'].LearningDelivery WHERE FundModel = '+ @FModel + CHAR(13)
	exec(@output)
END
GO
