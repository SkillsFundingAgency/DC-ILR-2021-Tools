...
from	(
			...
			select	ULN
			from	${runmode.inputsource}.LearnerDestinationandProgression
...
from	[${FCS-Contracts.servername}].[${FCS-Contracts.databasename}].[dbo].[vw_ContractAllocation]
			inner join ${runmode.inputsource}.LearningProvider
...
from	(
			...
			from	${runmode.inputsource}.LearningDeliveryWorkPlacement
			union 
			select	EmpId
			from	${runmode.inputsource}.LearnerEmploymentStatus
		) as FromTables
...
from	${runmode.inputsource}.PostcodesList
