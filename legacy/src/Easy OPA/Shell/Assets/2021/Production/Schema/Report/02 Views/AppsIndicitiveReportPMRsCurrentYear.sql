 IF OBJECT_ID('Report.GetValidAppFinRecordCurrentYear') IS NOT NULL
BEGIN
	DROP FUNCTION Report.GetValidAppFinRecordCurrentYear
END
GO

CREATE FUNCTION [Report].[GetValidAppFinRecordCurrentYear] (@LearnRefNumber varchar(12),@AimSeqNumber int,@AFinCode int)
RETURNS INT
AS 
BEGIN
DECLARE @summed int
	...
	AND AFinDate >= '${YearStartDate}' and AFinDate <= '${YearEndDate}'
...
INNER JOIN 
	...
	--Handles same PMR values for same price episodes
		...
		(
			...
			[Report].[GetValidAppFinRecordCurrentYear] (pe.LearnRefNumber,pe.PriceEpisodeAimSeqNumber ,1) AS TNP1, 
			[Report].[GetValidAppFinRecordCurrentYear] (pe.LearnRefNumber,pe.PriceEpisodeAimSeqNumber ,2) AS TNP2,
			[Report].[GetValidAppFinRecordCurrentYear] (pe.LearnRefNumber,pe.PriceEpisodeAimSeqNumber ,3) AS TNP3, 
			[Report].[GetValidAppFinRecordCurrentYear] (pe.LearnRefNumber,pe.PriceEpisodeAimSeqNumber ,4) AS TNP4			 			
