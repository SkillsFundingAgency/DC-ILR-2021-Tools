IF OBJECT_ID('GetOLASSEASCancellationCostsFunding') IS NOT NULL
BEGIN
    DROP FUNCTION  GetOLASSEASCancellationCostsFunding
END
GO

CREATE FUNCTION GetOLASSEASCancellationCostsFunding (@collectionPeriod int) 
RETURNS TABLE 
AS 
RETURN
(
 ...
 (FundCalcApr + FundCalcMay + FundCalcJun + FundCalcJul) as AprToJul,* FROM (
	...
	SELECT	(CASE WHEN Period = 1 THEN CAST(Value AS DECIMAL(15,5)) END) AS FundCalcAug,
		...
		FROM (SELECT val.CollectionPeriod as Period, SUM(val.CancellationCosts) AS Value 
