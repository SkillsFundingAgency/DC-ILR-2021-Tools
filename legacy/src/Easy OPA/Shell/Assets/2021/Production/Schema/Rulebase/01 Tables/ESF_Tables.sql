...
go
  if object_id('Rulebase.ESF_Cases', 'u') is not null begin drop table Rulebase.ESF_Cases
end
go
...
go
  create table Rulebase.ESF_global (UKPRN int, RulebaseVersion varchar(10),)
go
...
go
  create table Rulebase.ESF_LearningDelivery (
    ...
    [Restart] bit,
    WeightedRateFromESOL decimal(10, 5) primary key clustered (LearnRefNumber asc, AimSeqNumber asc)
  )
go
...
go
  if object_id(
    ...
    'u'
  ) is not null begin drop table Rulebase.ESF_LearningDeliveryDeliverable_Period
end
go
...
go
  if object_id(
    ...
    'u'
  ) is not null begin drop table Rulebase.ESF_LearningDeliveryDeliverable_PeriodisedValues
end
go
...
go
  create table Rulebase.ESF_DPOutcome (
    ...
    ReachedTwelveMonthPoint bit primary key clustered (
      LearnRefNumber asc,
      OutCode asc,
      OutType asc,
      OutStartDate asc
...
go
  if object_id('[Rulebase].[ESF_LearnerEmploymentStatus]', 'u') is not null drop table [Rulebase].[ESF_LearnerEmploymentStatus] create table [Rulebase].[ESF_LearnerEmploymentStatus] (
    UKPRN int,
    [LearnRefNumber] varchar(12),
    [LearnDelLearnerEmpAtStart] bit,
    primary key clustered ([LearnRefNumber] asc)
  )
go
