if object_id('Rulebase.FM35_Get_Cases','p') is not null
begin
	drop procedure Rulebase.FM35_Get_Cases
end
go

create procedure Rulebase.FM35_Get_Cases as
begin
	set nocount on
	select	CaseData
	from	Rulebase.FM35_Cases
end
go

if object_id('Rulebase.FM35_Insert_Cases','p') is not null
begin
	drop procedure Rulebase.FM35_Insert_Cases
end
go

create procedure Rulebase.FM35_Insert_Cases as
begin
	set nocount ON

	declare @UKPRN int         
	select	@UKPRN = UKPRN
	from	(select top 1 UKPRN from Valid.LearningProvider) as UnrequiredAlias

	insert into Rulebase.FM35_Cases (
		LearnRefNumber,
		CaseData
	)
    select	cntrl.LearnRefNumber,
			convert(xml, (	select	@UKPRN as [@UKPRN],
									lv.CurrentVersion as [@LARSVersion],
									ov.CurrentVersion as [@OrgVersion],
									-- PostcodeDisadvantageVersion
									(select	EffectiveFrom as [@OrgFundEffectiveFrom],
									...
									from	Reference.Org_Funding
									where	UKPRN = @UKPRN
									and		FundingFactorType = 'Adult Skills'
									...
									(select	l.LearnRefNumber as [@LearnRefNumber],
											...
											(select	EmpId as [@EmpId],
													...
													(select	EffectiveFrom as [@LargeEmpEffectiveFrom],
															EffectiveTo as [@LargeEmpEffectiveTo]
													from	Reference.LargeEmployers
													where	ERN = EmpId
											...
											(select	ld.AimSeqNumber as [@AimSeqNumber],
                                                    ...
                                                    (select	LearnDelFAMType as [@LearnDelFAMType],
                                                    ...
                                                    (select	CategoryRef as [@LearnDelCatRef],
															EffectiveFrom as [@LearnDelCatDateFrom],
                                                            EffectiveTo as [@LearnDelCatDateTo]
													from	Reference.LARS_LearningDeliveryCategory
                                                    ...
                                                    (select	AreaCostFactor as [@AreaCosFactor],
                                            ...
                                            from	Valid.LearningDeliveryDenormTbl as ld
														...
														left join Reference.LARS_FrameworkAims as lars_fa 
							...
							where	globalLearner.LearnRefNumber = cntrl.LearnRefNumber
							for xml path ('global'), type))
					from	(select	distinct
									filterLD.LearnRefNumber
							from	Valid.LearningDelivery as filterLD
							where	filterLD.FundModel = 35) as cntrl
end
go

if object_id('Rulebase.FM35_Insert_global','p') is not null
begin
	drop procedure Rulebase.FM35_Insert_global
end
go
...
begin
	set nocount on
	insert into Rulebase.FM35_global (
		UKPRN,
		CurFundYr,
		LARSVersion,
		OrgVersion,
...
if object_id('Rulebase.FM35_Insert_LearningDelivery','p') is not null
begin
	drop procedure Rulebase.FM35_Insert_LearningDelivery
end
go

create procedure Rulebase.FM35_Insert_LearningDelivery (
	@LearnRefNumber varchar(12),				-- passed through, key
	@AimSeqNumber int,							-- passed through, key
	@AchApplicDate date,
	@Achieved bit,
	@AchieveElement decimal(10,5),
	@AchievePayElig bit,
	@AchievePayment decimal(10,5),				-- periodised
	@AchievePayPct decimal(10,5),				-- periodised
	@AchievePayPctPreTrans decimal(10,5),
	@AchievePayPctTrans decimal(10,5),			-- periodised
	...
	@BalancePayment decimal(10,5),				-- periodised
	@BalancePaymentUncapped decimal(10,5),		-- periodised
	@BalancePct decimal(10,5),					-- periodised
	@BalancePctTrans decimal(10,5),				-- periodised
	@BalInstalmPreTrans int,
	@BaseValueUnweight decimal(10,5),
	@CapFactor decimal(10,5),
	@DisUpFactAdj decimal(10,5),
	@EmpOutcomePay decimal(10,5),				-- periodised
	@EmpOutcomePayElig bit,
	@EmpOutcomePct decimal(10,5),				-- periodised
	@EmpOutcomePctHeldBackTrans decimal(10,5),
	@EmpOutcomePctPreTrans decimal(10,5),
	@EmpOutcomePctTrans decimal(10,5),			-- periodised
	@EmpRespOth bit,
	@ESOL bit,
	@FullyFund bit,
	@FundLine varchar(100),
	@FundStart bit,
	@InstPerPeriod int,							-- periodised
	@LargeEmployerID int,
	@LargeEmployerFM35Fctr decimal(10,5),
	@LargeEmployerStatusDate date,
	@LearnDelFundOrgCode varchar(50),			-- new for 1920?
	@LearnSuppFund bit,							-- periodised ?? can this turn on and off for each period?
	@LearnSuppFundCash decimal(10,5),			-- periodised
	@LTRCUpliftFctr decimal(10,5),
	@NonGovCont decimal(10,5),
	@OLASSCustody bit,
	@OnProgPayment decimal(10,5),				-- periodised
	@OnProgPaymentUncapped decimal(10,5),		-- periodised
	@OnProgPayPct decimal(10,5),				-- periodised
	@OnProgPayPctPreTrans decimal(10,5),
	@OnProgPayPctTrans decimal(10,5),			-- periodised
	...
	@Traineeship bit,
	@Trans bit,
	@TransInstPerPeriod int,					-- periodised
...
begin
	set nocount on
	insert into Rulebase.FM35_LearningDelivery (
		LearnRefNumber,
		AimSeqNumber,
		AchApplicDate,
		Achieved,
...
if object_id('Rulebase.FM35_Insert_LearningDelivery_PeriodisedValues','p') is not null
begin
	drop procedure Rulebase.FM35_Insert_LearningDelivery_PeriodisedValues
end
go
...
begin
	set nocount on
	insert into Rulebase.FM35_LearningDelivery_PeriodisedValues (
		LearnRefNumber,
		AimSeqNumber,
		AttributeName,
		Period_1,
...
if object_id('Rulebase.FM35_PivotTemporals_LearningDelivery','p') is not null
begin
	drop procedure Rulebase.FM35_PivotTemporals_LearningDelivery
end
go

create procedure Rulebase.FM35_PivotTemporals_LearningDelivery as
begin
	truncate table Rulebase.FM35_LearningDelivery_Period

	insert into Rulebase.FM35_LearningDelivery_Period
