IF OBJECT_ID('GetLSFFundingForType') IS NOT NULL
BEGIN
    DROP FUNCTION  GetLSFFundingForType
END
GO
CREATE FUNCTION GetLSFFundingForType (@learnerType VARCHAR(MAX), @collectionPeriod int) 
RETURNS TABLE 
AS 
RETURN
(
 ...
 (FundCalcApr + FundCalcMay + FundCalcJun + FundCalcJul) as AprToJul,* FROM (
	...
	SELECT
		...
		LEFT JOIN [Rulebase].[FM35_LearningDelivery_PeriodisedValues] sfap ON sfap.LearnRefNumber = sfaf.LearnRefNumber and sfap.AimSeqNumber =sfaf.AimSeqNumber
		WHERE sfaf.FundLine IN (SELECT * FROM ConvertCSVToTable(@learnerType)) and sfap.[AttributeName] ='LearnSuppFundCash'
