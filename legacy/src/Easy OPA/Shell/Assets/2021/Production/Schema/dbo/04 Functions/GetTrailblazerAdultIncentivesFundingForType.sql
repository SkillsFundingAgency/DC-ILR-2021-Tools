IF OBJECT_ID('GetTrailblazerAdultIncentivesFundingForType') IS NOT NULL
BEGIN
    DROP FUNCTION  GetTrailblazerAdultIncentivesFundingForType
END
GO

CREATE FUNCTION GetTrailblazerAdultIncentivesFundingForType (@learnerType VARCHAR(MAX), @collectionPeriod int)
RETURNS TABLE 
AS 
RETURN
(
 ...
 (FundCalcApr + FundCalcMay + FundCalcJun + FundCalcJul) as AprToJul,* FROM (
	...
	SELECT
		...
		WHERE tbf.FundLine = @learnerType and tbfp.AttributeName in ('AchPayment','SmallBusPayment','YoungAppPayment')
