...
--Insert statement that inserts all the attribute values where their length is outside the specified length.
	...
	select
		null, 'The value returned for PHours does not contain a value between 0 and 9999', 'PHours=''' + cast(PHours as varchar(255)) + ''',length=''' + cast(len(cast(PHours as varchar(255))) as varchar(255))+'''', [LearnRefNumber], 'FD_PHours_AL'
	...
	where
		PHours is not null 
		and PHours not between 0 and 9999
