...
begin
	insert into Rulebase.ESF_Cases (
		LearnRefNumber,
		CaseData
	)
			...
			convert(xml,(	select	LearningProvider.UKPRN as [@UKPRN],
									(select	l.DateOfBirth as [@DateOfBirth],
											...
											(	select	ld.LearnAimRef as [@LearnAimRef],
														...
														(select	ESFData.ExternalDeliverableCode as [@ESFDeliverableCode],
														...
														from	(	select	vw_ContractDescription.contractEndDate,
																	...
																	from	Reference.DeliverableCodeMappings
														...
														where	ESFData.contractAllocationNumber = ld.ConRefNumber
														for xml path ('ESFData'), type),
														(select	lf.FundingCategory as [@LARSFundingCategory],
														...
														from	Reference.LARS_Funding as lf
														where	lf.LearnAimRef = ld.LearnAimRef
														for xml path ('LearningDeliveryLARSFunding'), type),
														(select	lav.BasicSkillsType as [@LearnDelAnnValBasicSkillsTypeCode],
																lav.EffectiveFrom as [@LearnDelAnnValDateFrom],
																lav.EffectiveTo as [@LearnDelAnnValDateTo]
														from	Reference.LARS_AnnualValue as lav
														where	lav.LearnAimRef = ld.LearnAimRef
														for xml path ('LearningDeliveryAnnualValue'), type),
														(select	SFA_PostcodeAreaCost.EffectiveFrom as [@AreaCosEffectiveFrom],
														...
														where	SFA_PostcodeAreaCost.Postcode = ld.DelLocPostCode
												...
												from	Valid.LearningDelivery as ld
															...
															left join (select	Lot.CalcMethod,
	...
	from	Valid.Learner as ControllingTable
				inner join	(select	distinct
							...
							where	LearningDelivery.FundModel = 70) as Filter_LearningDelivery
...
begin
	insert into Rulebase.ESF_global (
		UKPRN,
		RulebaseVersion
	) values (
		@UKPRN,
...
begin
	-- nothing to do, but i has to be here for the OPA black box
	set nocount on
end
go
...
begin
	insert into Rulebase.ESF_LearningDelivery (
		LearnRefNumber,
		AimSeqNumber,
		Achieved,
		AddProgCostElig,
...
begin
	insert into Rulebase.ESF_LearningDeliveryDeliverable (
		LearnRefNumber,
		AimSeqNumber,
		DeliverableCode,
		DeliverableUnitCost
...
begin
	insert into Rulebase.ESF_LearningDeliveryDeliverable_PeriodisedValues (
		LearnRefNumber,
		AimSeqNumber,
		DeliverableCode,
		AttributeName,
...
begin
	...
	insert into Rulebase.ESF_LearningDeliveryDeliverable_Period (
		LearnRefNumber,
		AimSeqNumber,
		DeliverableCode,
		[Period],
	...
	select	LearnRefNumber,
			...
			DeliverableCode,
			[Period],
			max(case AttributeName when 'AchievementEarnings' then [Value] else null end) as AchievementEarnings,
			max(case AttributeName when 'AdditionalProgCostEarnings' then [Value] else null end) as AdditionalProgCostEarnings,
			max(case AttributeName when 'DeliverableVolume' then [Value] else null end) as DeliverableVolume,
			max(case AttributeName when 'ProgressionEarnings' then [Value] else null end) as ProgressionEarnings,
			max(case AttributeName when 'ReportingVolume' then [Value] else null end) as ReportingVolume,
			max(case AttributeName when 'StartEarnings' then [Value] else null end) as StartEarnings
...
begin
	insert into Rulebase.ESF_DPOutcome (
		LearnRefNumber,
		OutCode,
		OutType,
		OutStartDate,
